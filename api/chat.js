...
// Está organizado em 5 seções editáveis + código interno:
//
...
//
//   SEÇÃO 4 — PALAVRAS-CHAVE DO APP (funcionalidades internas)
//             ↳ Edite aqui para que a IA explique o próprio sistema
//
...
// - Para adicionar uma nova regra: copie uma linha [REGRA X] e cole abaixo
// - Para remover uma regra: apague a linha correspondente
...
[REGRA 10] Para dúvidas sobre o APP (como usar, o que faz cada tela,
           como atualizar cadastro, onde baixar documentos): explicar
           de forma simples com base nas informações do documento
           "Funcionalidades do App UNASLAF" da base de conhecimento.
...
//   detecta "abono de ponto", "abono do ponto", etc.
//
...
const DETECTORES = {
  ...
  // Tabela completa de todos os processos
  ...
  // Funcionalidades do APP — ativa quando o associado tem dúvida sobre o sistema
  // Adicione palavras que indiquem dúvida sobre como usar o app
  funcionalidadesApp: /como.*usar|como.*funciona|o que.*app|o que.*sistema|tela|menu|cadastro.*app|atualizar.*dados|meu.*cadastro|baixar.*documento|repositório|histórico.*app|como.*acessar|esqueci.*cpf|login|entrar.*app|como.*chat|assistente|como.*funciona.*ia/i,
...
//   app_funcionalidades  ← novo: explica o app ao associado
// =============================================================================
...
// =============================================================================
// Permite carregar documentos extras do Google Drive sem precisar alterar
// o código. Útil para comunicados e notas que mudam com frequência.
//
// Esta seção funciona automaticamente — não é necessário editar.
// Para adicionar conteúdo dinâmico, basta subir um arquivo .txt ou
// Google Doc na pasta de contexto do Drive configurada no Vercel.
// =============================================================================

async function getDriveContext(baseUrl) {
  try {
    const res = await fetch(`${baseUrl}/api/context-drive`);
    if (!res.ok) return '';
...
//   4000 = resposta muito longa (pode ficar mais lento)
//
...
function selectContext(msg) {
  ...
  // Dúvida sobre o APP → carrega documento de funcionalidades
  if (DETECTORES.funcionalidadesApp.test(m)) return getDoc('app_funcionalidades');
...
export default async function handler(req, res) {
  ...
  if (!authHeader) return res.status(401).json({ error: 'Sessão expirada. Faça login novamente.' });

  const { messages, userName, userCpf, baseUrl } = req.body;
